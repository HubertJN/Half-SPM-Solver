...
CONTAINS
  ...
  FUNCTION crank_nicholson(n, D, R, volPer, iapp, F, L, Rg, T, , K, maxCon, c0, timeArr) RESULT(voltArray)
    ...
    voltConRtf = (2.0_REAL64*Rg*T)/F

    A = 0.0_REAL64
    B = 0.0_REAL64

    ai = dt*modD/(2.0_REAL64*dr*dr)
   ...
   !smooth boundary conditions at r=0
    ...
    rhsConst = 2.0_REAL64*fluxParam*(dt + dt/dr)
    !------------------------------------------------------------------------------------------
    ...
    DO time = 1, totalSim 
      ...
      U_scalar = -0.8090_REAL64*div_const + 4.4875_REAL64 - 0.0428_REAL64*TANH(18.5138_REAL64*(div_const-0.5542_REAL64)) &
              -17.7326_REAL64*TANH(15.7890_REAL64*(div_const-0.3117_REAL64)) & 
              +17.5842_REAL64*TANH(15.9308_REAL64*(div_const-0.3120_REAL64))
      !----------------------------Calculate arsinh part: ----------------------------------------------
      arsinh = F*K*SQRT(div_const - (div_const**2))
      arsinh = voltConIal/arsinh
      arsinh = ASINH(arsinh)
      !----------------------------Calculate the voltage: ----------------------------------------------
      voltArray(time-1) = U_scalar - (voltConRtf*arsinh)
    END DO
